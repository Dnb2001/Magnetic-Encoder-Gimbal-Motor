...
void InitEPwm1()  // 周期 相位 计数器初始值 增减模式 时钟 CMPA初值 动作 死区 触发ADC 同步
{
    ...
    EPwm1Regs.DBRED = 450;    // 上升沿死区时间 (RED) - 450个时钟周期 (约 3us)
    EPwm1Regs.DBFED = 450;    // 下降沿死区时间 (FED) - 450个时钟周期
...
void InitEPwm2()
{
    ...
    EPwm2Regs.DBRED = 450;    // 上升沿死区时间 (RED) - 4500个时钟周期 (约 3us)
    EPwm2Regs.DBFED = 450;    // 下降沿死区时间 (FED) - 450个时钟周期
    // --- 【关键：同步设置】 ---
...
void InitEPwm3()
{
    ...
    EPwm3Regs.DBRED = 450;    // 上升沿死区时间 (RED) - 450个时钟周期 (约 3us)
    EPwm3Regs.DBFED = 450;    // 下降沿死区时间 (FED) - 450个时钟周期
    // --- 【关键：同步设置】 ---
